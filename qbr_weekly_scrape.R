...
#pause is because espns servers are very sensitive
...
qbr_11 <- qbr_grab(2011, base_url = baseurl)
qbr_12 <- qbr_grab(2012, base_url = baseurl)
qbr_13 <- qbr_grab(2013, base_url = baseurl)
qbr_14 <- qbr_grab(2014, base_url = baseurl)
qbr_15 <- qbr_grab(2015, base_url = baseurl)
...
qbr_df$QBshort <- qbr_df$QB %>%
  str_replace_all(fixed("."), "") %>%
  str_replace_all(" Jr", "") %>%
  str_split(" ") %>%
  map_chr(~ str_c(tail(.x, 1), str_sub(.x[1], 1, 1), sep=", "))
#get offense points, know who won
...
qbr_df$Points_For <- ifelse(qbr_df$Win == "W", winning_points, losing_points) 
