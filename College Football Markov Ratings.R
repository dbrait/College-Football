...
GetScores <- function(yearvec, homeadjust=T, fbs=T){
  ...
  for(year in yearvec){
    if(year == 2014){
      rawscores <- readHTMLTable(paste0("http://www.sports-reference.com/cfb/years", years, "-schedule.html"), header=F, stringsAsFactors=F, which=1)
...
Wrapper <- function(years){
  ...
  ctmcratings <- RatingsPredict(scores,year,method="CTMC",best="large",startweek=5)
  masseyratings <- RatingsPredict(scores,year,method="Massey",best="large",startweek=5)
